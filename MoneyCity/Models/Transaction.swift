...
public final class Transaction: Identifiable {
    ...
    public init(
        ...
        self.id = id
        self.amount = amount.isFinite ? amount : 0.0
        ...
        self.confidenceScore = confidenceScore.isFinite ? min(1.0, max(0.0, confidenceScore)) : 1.0
        self.isManual = isManual
        self.isConfirmed = isConfirmed
        self.note = note.map { InputSanitizer.sanitizeMultiline($0, maxLength: InputSanitizer.maxNoteLength) }
        self.buildingIdRaw = buildingId.map { InputSanitizer.sanitizeIdentifier($0) }
        self.originalAmount = originalAmount.flatMap { $0.isFinite ? $0 : nil }
