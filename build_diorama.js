...
const cityEncountersJs = fs.readFileSync(path.join(__dirname, 'city_v2_encounters.js'), 'utf8');
...
<head>
  ...
  <style>
    ...
    * { font-family: -apple-system, BlinkMacSystemFont, "SF Pro Rounded", "SF Pro Text", "Helvetica Neue", "Arial Hebrew", "Arial", sans-serif; }
    ...
    /* 💸 Expense Spatial Amount Bubble */
    .diorama-expense-bubble {
      position: absolute;
      top: 0; left: 0;
      display: inline-flex;
      align-items: center;
      ...
      border-radius: 999px;
      box-shadow: 0 4px 14px rgba(15, 23, 42, 0.10), 0 1px 3px rgba(15, 23, 42, 0.05);
    ...
    .diorama-expense-bubble.active {
      opacity: 1;
      transform: translate3d(-50%, -100%, 0) scale(1.0);
    }
    .diorama-expense-bubble.closing {
      opacity: 0;
      transform: translate3d(-50%, -135%, 0) scale(0.94);
      transition: opacity 0.40s ease, transform 0.40s cubic-bezier(0.25, 1, 0.5, 1);
    }
    .diorama-expense-bubble .sign-pos {
      color: #16A34A;
      margin-inline-end: 2px;
    }
    .diorama-expense-bubble .sign-neg {
      color: #DC2626;
      margin-inline-end: 2px;
    }
    @media (prefers-reduced-motion: reduce) {
      .diorama-expense-bubble {
        transform: translate3d(-50%, -100%, 0) scale(1.0) !important;
  ...
  <script>
    ...
    function addBenchAt(bx, bz, rotY, y) {
      ...
      for (let slat = 0; slat < 3; slat++) {
        g.add(mesh(new THREE.BoxGeometry(1.05, 0.055, 0.085), M_WOOD, 0, 0.30, -0.105 + slat * 0.105, false, false));
        g.add(mesh(new THREE.BoxGeometry(1.05, 0.065, 0.045), M_WOOD, 0, 0.41 + slat * 0.08, -0.15, false, false));
      }
      [-0.44, 0.44].forEach(function (x) {
        g.add(mesh(new THREE.BoxGeometry(0.055, 0.54, 0.055), M_DARKFRAME, x, 0.27, -0.15, false, false));
        g.add(mesh(new THREE.BoxGeometry(0.055, 0.29, 0.055), M_DARKFRAME, x, 0.145, 0.12, false, false));
        g.add(mesh(new THREE.BoxGeometry(0.055, 0.045, 0.34), M_DARKFRAME, x, 0.43, 0, false, false));
      });
      packRigidModel(g);
      root.add(g);
      return g;
    ...
    // ────────────────────────────────────────────────────────────────
    // Backward-compat skin materials (couriers, seated guests, companions).
    const M_SKIN  = mat(0xF6C88A, 0.82);
    const M_SKIN2 = mat(0xC98C56, 0.82);

    // ─────────────────────────────────────────────────────────────────
    // CITIZEN APPEARANCE SYSTEM v1
    // Additive layer over the existing rig. buildAppearanceProfile(key)
    // is a pure function — same key always → same profile, no side effects.
    // ─────────────────────────────────────────────────────────────────
    ...
    // Material cache — shared, never mutated after creation.
    // Companions use makeFigure without opts.appearance → get fresh mat() calls,
    // so companionFigure's rewardOwned tagging never touches the cache.
    const citizenMatCache = new Map();
    function citizenMat(hex, roughness) {
      const k = hex + '-' + (roughness || 0.82);
      if (!citizenMatCache.has(k)) citizenMatCache.set(k, mat(hex, roughness || 0.82));
      return citizenMatCache.get(k);
    }
    ...
    const CITIZEN_GEO = (function () {
      ...
      return {
        ...
        hairBobChunk: new THREE.BoxGeometry(0.080, 0.056, 0.046),
        hairLongBase: new THREE.SphereGeometry(HR,        10, 8, 0, Math.PI * 2, 0, Math.PI * 0.55),
        hairLongExt:  new THREE.CylinderGeometry(0.032, 0.016, 0.12, 6),
        hairBunBase:  new THREE.SphereGeometry(HR * 0.97, 8, 6, 0, Math.PI * 2, 0, Math.PI * 0.55),
        hairBunKnot:  new THREE.SphereGeometry(0.026, 6, 5),
        hairPonyBase: new THREE.SphereGeometry(HR * 0.97, 8, 6, 0, Math.PI * 2, 0, Math.PI * 0.55),
        hairPonyTail: new THREE.CylinderGeometry(0.016, 0.009, 0.088, 5),
        hairShaved:   new THREE.SphereGeometry(HR * 0.93, 8, 5, 0, Math.PI * 2, 0, Math.PI * 0.32),
        accBackpackBody: new THREE.BoxGeometry(0.16, 0.19, 0.085),
        accBackpackStr:  new THREE.BoxGeometry(0.02, 0.23, 0.02),
        accCrossbody:    new THREE.BoxGeometry(0.10, 0.12, 0.06),
        accTote:         new THREE.BoxGeometry(0.13, 0.14, 0.05),
        accCapDome:      new THREE.SphereGeometry(0.09, 10, 8, 0, Math.PI * 2, 0, Math.PI * 0.5),
        accCapBrim:      new THREE.BoxGeometry(0.14, 0.016, 0.08),
        accHpBand:       new THREE.BoxGeometry(0.195, 0.010, 0.010),
        accHpCup:        new THREE.CylinderGeometry(0.021, 0.021, 0.015, 7),
        jacketShoulder:  new THREE.BoxGeometry(0.305, 0.044, 0.148),
        looseBtmFlare:   new THREE.CylinderGeometry(0.118, 0.094, 0.065, 7)
      };
    })();
    ...
    // Accessory builder — at most one accessory, all in torso-child group.
    function addAccessoryToGroup(grp, accessory, topColorHex) {
      if (!accessory) return;
      const darkMat = citizenMat(0x2E2E33, 0.85);
      const aMat    = citizenMat(topColorHex, 0.78);
      switch (accessory) {
        case 'backpack':
          grp.add(mesh(CITIZEN_GEO.accBackpackBody, aMat, 0, 0.01, -0.12, false, false));
          [-0.06, 0.06].forEach(function (x) {
            grp.add(mesh(CITIZEN_GEO.accBackpackStr, aMat, x, 0.02, 0.072, false, false));
          }); break;
        case 'crossbody': {
          const cb = mesh(CITIZEN_GEO.accCrossbody, aMat, -0.115, -0.02, -0.042, false, false);
          cb.rotation.z = 0.14; grp.add(cb); break;
        }
        case 'tote':
          grp.add(mesh(CITIZEN_GEO.accTote, aMat, 0.125, -0.018, -0.040, false, false)); break;
        case 'cap':
          grp.add(mesh(CITIZEN_GEO.accCapDome, darkMat, 0, 0.305, 0, false, false));
          grp.add(mesh(CITIZEN_GEO.accCapBrim, darkMat, 0, 0.305, 0.085, false, false)); break;
        case 'headphones': {
          grp.add(mesh(CITIZEN_GEO.accHpBand, darkMat, 0, 0.352, 0, false, false));
          const cupL = mesh(CITIZEN_GEO.accHpCup, darkMat, -0.092, 0.267, 0, false, false);
          cupL.rotation.z = Math.PI / 2;
          const cupR = mesh(CITIZEN_GEO.accHpCup, darkMat,  0.092, 0.267, 0, false, false);
          cupR.rotation.z = Math.PI / 2;
          grp.add(cupL, cupR); break;
        }
      }
    }

    // Clothing silhouette detail — additive mesh on torso. basic = no extra mesh.
    function addClothingDetail(torso, topStyle, topMat) {
      if (topStyle === 'jacket') {
        torso.add(mesh(CITIZEN_GEO.jacketShoulder, citizenMat(0x252528, 0.82), 0, 0.108, 0, false, false));
      } else if (topStyle === 'looseBtm') {
        torso.add(mesh(CITIZEN_GEO.looseBtmFlare, topMat, 0, -0.172, 0, false, false));
      }
      // longTop handled by body cylinder y-scale tweak inside makeFigure; no extra mesh.
    }

    // Pure appearance profile builder — no side effects, no global reads.
    ...
    const recentPopulationProfiles = [];
    const RECENT_PROFILE_WINDOW = 6;

    // ─────────────────────────────────────────────────────────────────
    // makeFigure — when opts.appearance is absent: behavior is identical
    // to the original. When opts.appearance is provided, the diversity
    // layer activates. All joint refs in userData remain unchanged.
    // ─────────────────────────────────────────────────────────────────
    function makeFigure(opts) {
      ...
      const shirt = appearance ? citizenMat(appearance.topColor, 0.76)
                               : mat(opts.shirt || 0x3B82F6, 0.76);
      const pants = appearance ? citizenMat(appearance.bottomColor, 0.86)
                               : mat(opts.pants || 0x1E293B, 0.86);
      const skin  = appearance ? CITIZEN_SKIN_MATS[appearance.skinTone]
                               : (opts.dark ? M_SKIN2 : M_SKIN);
      ...
      const hairGrp = new THREE.Group(); hairGrp.name = 'hair'; torso.add(hairGrp);
      if (appearance) {
        addHairToGroup(hairGrp, appearance.hairStyle, appearance.hairColor);
      } else if (opts.hair !== false) {
        hairGrp.add(mesh(new THREE.SphereGeometry(0.083, 10, 8, 0, Math.PI * 2, 0, Math.PI * 0.55),
          mat(opts.hair || 0x3B2412, 0.9), 0, 0.291, -0.004, false, false));
      }

      // Cap — legacy path only; appearance system uses the accessory group.
      if (opts.cap && !appearance) {
      ...
      // Accessory / bag group.
      const accGrp = new THREE.Group(); accGrp.name = 'accessory'; torso.add(accGrp);

      // Clothing detail group — separate replaceable group so pool reuse can clear and
      // re-apply topStyle without affecting any other torso children (joints, hair, etc.)
      ...
      // Arms — track upper-arm AND forearm/hand skin meshes for pool reuse.
      ...
      // Legs — track thigh/shin meshes for material update on pool reuse.
      ...
      // Body scale for diversity — feet at local y≈0 → stays grounded on Y_WALK.
      if (appearance) {
        fig.scale.set(appearance.widthScale, appearance.heightScale, appearance.widthScale);
      }

      fig.userData = {
        // Existing joint refs — all animation/encounter code uses these unchanged.
    ...
    const CHARACTER = {
      ...
      shop_boutique: function (g, w, d) {
        ...
        const goldAcc = mat(0xD4AF37, 0.3, 0.7);
        const bagOrange = mat(0xEA580C, 0.75);
        const bagBlack = mat(0x18181B, 0.85);

        // Window Showcase Displays (inside behind glass)
        [-w * 0.28, w * 0.28].forEach(function (mx, i) {
          const m = new THREE.Group(); m.position.set(mx, 0, d / 2 + 0.18); g.add(m);
          // Elevated marble display podium
          m.add(mesh(roundedBox(0.40, 0.12, 0.30, 0.03), M_CREAM, 0, 0.06, 0, false, false));
          m.add(mesh(new THREE.CylinderGeometry(0.11, 0.12, 0.02, 12), goldAcc, 0, 0.13, 0, false, false));
          m.add(mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.34, 8), goldAcc, 0, 0.30, 0, false, false));
          // Mannequin torso (couture gown / tailored suit)
          const dressMat = mat(i === 0 ? 0x0F766E : 0xBE185D, 0.75);
          m.add(mesh(roundedBox(0.18, 0.28, 0.12, 0.04), dressMat, 0, 0.58, 0, false, false));
          m.add(mesh(new THREE.SphereGeometry(0.055, 8, 8), M_CREAM, 0, 0.78, 0, false, false));
          // Shaped garments make the display legible at city scale.
          if (i === 0) {
            m.add(mesh(new THREE.CylinderGeometry(0.075, 0.17, 0.25, 8), dressMat, 0, 0.38, 0, false, false));
          } else {
            [-0.045, 0.045].forEach(function (x) {
              m.add(mesh(new THREE.BoxGeometry(0.065, 0.24, 0.09), dressMat, x, 0.35, 0, false, false));
            });
          }
          // Designer luxury shopping bags on display beside mannequin
          const bMat = i === 0 ? bagOrange : bagBlack;
          m.add(mesh(roundedBox(0.12, 0.16, 0.08, 0.02), bMat, 0.13, 0.20, 0.05, false, false));
          m.add(mesh(new THREE.TorusGeometry(0.03, 0.007, 4, 8, Math.PI), goldAcc, 0.13, 0.29, 0.05, false, false));
        });

        // Sidewalk entrance dressing: Twin architectural cube planters
        [-w * 0.38, w * 0.38].forEach(function (px) {
          const pl = new THREE.Group(); pl.position.set(px, 0, d / 2 + 0.38); g.add(pl);
          pl.add(mesh(roundedBox(0.26, 0.30, 0.26, 0.03), mat(0x18181B, 0.8), 0, 0.15, 0, false, false));
          pl.add(mesh(new THREE.SphereGeometry(0.14, 8, 8), M_STREET_LEAF, 0, 0.38, 0, false, false));
        });

        // Chic brass & black designer A-frame sidewalk stanchion
        const st = new THREE.Group(); st.position.set(-w * 0.20, 0, d / 2 + 0.65); g.add(st);
        st.rotation.y = 0.22;
        st.add(mesh(new THREE.BoxGeometry(0.03, 0.44, 0.03), goldAcc, -0.11, 0.22, 0, false, false));
        st.add(mesh(new THREE.BoxGeometry(0.03, 0.44, 0.03), goldAcc,  0.11, 0.22, 0, false, false));
    ...
    const CUSTOM_STALLS = {
      ...
      // 5. Shopping - Boutique (Chic Fashion Pop-Up Pavilion)
      shop_boutique: function (g, w, d) {
        const darkRoofMat = mat(0x18181B, 0.85);
        const goldMat = mat(0xD4AF37, 0.3, 0.7);
        // Clean white marble display deck
        g.add(mesh(roundedBox(1.50, 0.08, 1.30, 0.04), mat(0xFAFAF9, 0.8), 0, 0.04, 0));
        // Four slender gold/brass corner pillars
        [[-0.64, -0.54], [0.64, -0.54], [-0.64, 0.54], [0.64, 0.54]].forEach(function (p) {
          g.add(mesh(new THREE.CylinderGeometry(0.022, 0.022, 1.05, 8), goldMat, p[0], 0.55, p[1], false, false));
        });
        // Modern flat architectural canopy with gold fascia
        g.add(mesh(roundedBox(1.48, 0.10, 1.28, 0.03), darkRoofMat, 0, 1.10, 0));
        g.add(mesh(new THREE.BoxGeometry(1.50, 0.03, 0.02), goldMat, 0, 1.10, 0.65, false, false));
        // Polished brass designer clothing rail inside
        const rk = new THREE.Group(); rk.position.set(-0.20, 0.08, -0.10); g.add(rk);
        rk.add(mesh(new THREE.BoxGeometry(0.025, 0.025, 0.65), goldMat, 0, 0.56, 0, false, false));
        rk.add(mesh(new THREE.CylinderGeometry(0.015, 0.015, 0.56, 6), goldMat, 0, 0.28, -0.30, false, false));
        rk.add(mesh(new THREE.CylinderGeometry(0.015, 0.015, 0.56, 6), goldMat, 0, 0.28, 0.30, false, false));
        [0x0F766E, 0xBE185D, 0xD97706, 0x475569].forEach(function (c, i) {
          rk.add(mesh(new THREE.BoxGeometry(0.035, 0.30, 0.12), mat(c, 0.75), 0, 0.40, -0.22 + i * 0.15, false, false));
        });
        // Designer mannequin on marble base
        const man = new THREE.Group(); man.position.set(0.42, 0.08, 0.18); g.add(man);
        man.add(mesh(new THREE.CylinderGeometry(0.08, 0.10, 0.02, 10), goldMat, 0, 0.01, 0, false, false));
        man.add(mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.36, 6), goldMat, 0, 0.19, 0, false, false));
        man.add(mesh(roundedBox(0.16, 0.24, 0.11, 0.03), mat(0xBE185D, 0.75), 0, 0.46, 0, false, false));
        man.add(mesh(new THREE.SphereGeometry(0.05, 8, 8), M_CREAM, 0, 0.63, 0, false, false));
        // Luxury shopping bags beside mannequin
        man.add(mesh(roundedBox(0.10, 0.14, 0.07, 0.02), mat(0xEA580C, 0.8), 0.12, 0.08, 0.08, false, false));
      },
    ...
    const CUSTOM_FACADES = {
      ...
      // 5. Shopping - Boutique (Modern Luxury Flagship Storefront)
      shop_boutique: function (ground, w, d, cfg, bodyMat) {
        const frameMat = mat(0x18181B, 0.85);
        const goldMat = mat(0xD4AF37, 0.3, 0.7);
        const warmGlow = mat(0xFEF08A, 0.2, 0, 0xFDE047, 0.8);
        const gm = glassMaterial(0xBAE6FD); registerGlass(gm);

        // Projected display bays sit in front of the solid building shell.
        [-w * 0.28, w * 0.28].forEach(function (x) {
          ground.add(mesh(new THREE.BoxGeometry(0.49, 0.79, 0.035), frameMat, x, 0.48, d / 2 + 0.065, false, false));
          ground.add(mesh(new THREE.BoxGeometry(0.52, 0.07, 0.36), M_CREAM, x, 0.075, d / 2 + 0.18, false, false));
          ground.add(mesh(new THREE.BoxGeometry(0.49, 0.78, 0.018), gm, x, 0.49, d / 2 + 0.35, false, false));
          [-0.25, 0.25].forEach(function (dx) {
            ground.add(mesh(new THREE.BoxGeometry(0.026, 0.84, 0.33), goldMat, x + dx, 0.49, d / 2 + 0.19, false, false));
          });
        });

        // Matte black architectural framing & mullions
        [-w / 2 + 0.08, w / 2 - 0.08].forEach(function (px) {
          ground.add(mesh(new THREE.BoxGeometry(0.12, 0.98, 0.10), frameMat, px, 0.50, d / 2 + 0.06, false, false));
          ground.add(mesh(new THREE.BoxGeometry(0.14, 0.04, 0.12), goldMat, px, 0.98, d / 2 + 0.06, false, false));
        });
        // Central entrance framing mullions
        [-0.26, 0.26].forEach(function (mx) {
          ground.add(mesh(new THREE.BoxGeometry(0.04, 0.88, 0.06), frameMat, mx, 0.50, d / 2 + 0.05, false, false));
        });
        // Recessed double glass doors with tall gold architectural handles
        ground.add(mesh(new THREE.BoxGeometry(0.48, 0.84, 0.02), mat(0x0F172A, 0.85), 0, 0.44, d / 2 + 0.03, false, false));
        [-0.04, 0.04].forEach(function (hx) {
          ground.add(mesh(new THREE.CylinderGeometry(0.012, 0.012, 0.32, 6), goldMat, hx, 0.44, d / 2 + 0.07, false, false));
        });

        // Sleek razor-thin black & gold architectural canopy with warm recessed illumination
        ground.add(mesh(new THREE.BoxGeometry(w + 0.06, 0.04, 0.36), frameMat, 0, 0.98, d / 2 + 0.18, false, false));
        ground.add(mesh(new THREE.BoxGeometry(w + 0.08, 0.015, 0.02), goldMat, 0, 0.98, d / 2 + 0.36, false, false));
        // Recessed warm spotlights under canopy
        [-w * 0.28, 0, w * 0.28].forEach(function (lx) {
          ground.add(mesh(new THREE.CylinderGeometry(0.035, 0.04, 0.015, 8), warmGlow, lx, 0.955, d / 2 + 0.18, false, false));
        });
      },
    ...
    // ────────────────────────────────────────────────────────────────
    const CUSTOM_FLOORS = {
      // Boutique: Modern Luxury Fashion Atelier Upper Storeys
      shop_boutique: function (f, w, d, i, cfg, bodyMat) {
        const frameMat = mat(0x18181B, 0.85);
        const goldMat = mat(0xD4AF37, 0.3, 0.7);
        f.add(mesh(roundedBox(w - 0.04, FLOOR_H, d - 0.04, 0.06), bodyMat, 0, FLOOR_H / 2, 0));
        const gm = glassMaterial(cfg.glass); registerGlass(gm);
        // Floor-to-ceiling modern architectural glass gallery bays
        [-w * 0.24, w * 0.24].forEach(function (wx) {
          f.add(mesh(new THREE.BoxGeometry(0.46, 0.74, 0.05), gm, wx, FLOOR_H * 0.50, d / 2 + 0.02, false, false));
          // Sleek minimalist dark surround frame
          f.add(mesh(new THREE.BoxGeometry(0.50, 0.04, 0.08), frameMat, wx, FLOOR_H * 0.86, d / 2 + 0.03, false, false));
          f.add(mesh(new THREE.BoxGeometry(0.50, 0.04, 0.08), frameMat, wx, FLOOR_H * 0.14, d / 2 + 0.03, false, false));
          // Sleek vertical architectural gold accent fin beside each window
          f.add(mesh(new THREE.BoxGeometry(0.025, 0.82, 0.07), goldMat, wx + (wx > 0 ? 0.26 : -0.26), FLOOR_H * 0.50, d / 2 + 0.03, false, false));
          // Minimalist clear glass balustrade with fine gold top rail
          f.add(mesh(new THREE.BoxGeometry(0.46, 0.22, 0.02), gm, wx, FLOOR_H * 0.24, d / 2 + 0.06, false, false));
          f.add(mesh(new THREE.BoxGeometry(0.48, 0.02, 0.025), goldMat, wx, FLOOR_H * 0.35, d / 2 + 0.06, false, false));
        });
        // Stone cornice and side atelier windows give the visible return wall depth.
        f.add(mesh(roundedBox(w + 0.09, 0.085, d + 0.09, 0.025), M_CREAM, 0, FLOOR_H - 0.045, 0));
        [-1, 1].forEach(function (side) {
          [-d * 0.26, d * 0.26].forEach(function (z) {
            f.add(mesh(new THREE.BoxGeometry(0.06, 0.66, 0.56), frameMat, side * w / 2, FLOOR_H * 0.50, z, false, false));
            f.add(mesh(new THREE.BoxGeometry(0.025, 0.56, 0.46), gm, side * (w / 2 + 0.04), FLOOR_H * 0.50, z, false, false));
            f.add(mesh(new THREE.BoxGeometry(0.11, 0.045, 0.64), M_CREAM, side * (w / 2 + 0.025), FLOOR_H * 0.18, z, false, false));
            f.add(mesh(new THREE.BoxGeometry(0.03, 0.56, 0.025), goldMat, side * (w / 2 + 0.06), FLOOR_H * 0.50, z, false, false));
          });
        });
        // Central vertical architectural reveal / gold accent strip
        f.add(mesh(new THREE.BoxGeometry(0.03, FLOOR_H * 0.90, 0.04), goldMat, 0, FLOOR_H * 0.50, d / 2 + 0.02, false, false));
      },
    ...
    makeBuilding({
      ...
      body: 0xE8DDCF, roof: 0x18181B, accent: 0xD4AF37, glass: 0xBAE6FD, roofStyle: "deck",
      sign: { text: "FASHION", bg: "#18181B", fg: "#F59E0B", size: 26 }
    });
    makeBuilding({
    ...
    (function () {
      ...
      taxi.position.set(1.6, 0, -1.7); taxi.rotation.y = 0;
      parkedTaxi = taxi;
      const door = new THREE.Group(); door.position.set(-0.42, 0.27, 0.22);
      door.userData.rewardJoint = true;
      door.add(mesh(new THREE.BoxGeometry(0.035, 0.21, 0.46), mat(0xFACC15, 0.5), 0, 0.06, -0.23, false, false));
      door.add(mesh(new THREE.BoxGeometry(0.027, 0.22, 0.43), M_GLASS_BL, 0, 0.27, -0.23, false, false));
      door.add(mesh(new THREE.BoxGeometry(0.025, 0.025, 0.09), M_MULLION, -0.03, 0.10, -0.34, false, false));
      taxi.add(door); taxi.userData.passengerDoor = door;
    ...
    lakeMesh.userData = { id: "savings_sanctuary", district: "savings", name: "שמורת הטבע", amount: 0, trend: "קצב הוצאות חודשי" };
    ...
    for (let j = 0; j < 12; j++) {
      const z = -1.1 + j * 0.2;
      const rise = 0.12 + 0.15 * (1 - Math.pow(z / 1.2, 2));
      bridge.add(mesh(new THREE.BoxGeometry(0.78, 0.065, 0.185), M_WOOD, 0, rise, z));
    }
    [-0.36, 0.36].forEach(function (x) {
      for (let j = 0; j < 5; j++) {
        const z = -1.1 + j * 0.55;
        const rise = 0.12 + 0.15 * (1 - Math.pow(z / 1.2, 2));
        bridge.add(mesh(new THREE.BoxGeometry(0.055, 0.45, 0.055), M_WOOD, x, rise + 0.20, z));
        if (j < 4) {
          const nz = z + 0.55;
          const nextRise = 0.12 + 0.15 * (1 - Math.pow(nz / 1.2, 2));
          const rail = mesh(new THREE.BoxGeometry(0.06, 0.055, Math.hypot(0.55, nextRise - rise)), M_WOOD, x, (rise + nextRise) / 2 + 0.42, z + 0.275);
          rail.rotation.x = -Math.atan2(nextRise - rise, 0.55); bridge.add(rail);
        }
      }
    });
    ...
    // Conifer Pine & Deciduous Trees for city
    const M_PINE = mat(0x2F874D, 0.88);
    const M_LEAF = mat(0x4EAA50, 0.86);
    const M_LEAF_LIGHT = mat(0x79BF58, 0.88);

    // Dedicated Nature Reserve foliage & blooms (reacts dynamically to monthly spending pace)
    const M_PARK_PINE = mat(0x2D8048, 0.88);
    const M_PARK_LEAF = mat(0x4EAA50, 0.86);
    const M_PARK_LEAF_LIGHT = mat(0x78BF58, 0.88);
    const M_PARK_BLOOM = mat(0xFACC15, 0.90);

    const parkPlantings = [];
    function registerReservePlanting(obj, rank, kind, structural) {
      parkPlantings.push({ obj: obj, rank: rank, kind: kind, structural: structural,
        isReserve: true, baseScale: obj.scale.clone() });
    }
    // Resolve ownership using the meadow footprint in reserve-local coordinates.
    function belongsToReserve(px, pz, group) {
      if (!group || group === reserveGroup) return true;
      const point = new THREE.Vector3(px, Y_GRASS, pz);
      group.localToWorld(point); reserveGroup.worldToLocal(point);
      return Math.pow((point.x + 0.15) / (4.35 * 0.95), 2) +
        Math.pow((point.z - 0.1) / (4.35 * 0.90), 2) <= 1;
    }

    // Wildflowers scattered around the lake shore and footbridge
    const parkBloomGroup = new THREE.Group();
    reserveGroup.add(parkBloomGroup);
    const bloomGeo = new THREE.DodecahedronGeometry(0.065, 0);
    ...
    [[-1.8, 1.1], [-1.2, 1.8], [0.8, 1.8], [1.9, 1.2], [-0.9, -1.8], [0.5, -2.1], [1.8, -1.2], [-2.3, -0.8]].forEach(function (bp, patchIndex) {
      const patch = new THREE.Group(); patch.position.set(bp[0], Y_GRASS + 0.04, bp[1]); parkBloomGroup.add(patch);
      registerReservePlanting(patch, patchIndex / 8, "flowers", patchIndex < 2);
      for (let k = 0; k < 4; k++) {
        const ox = FLOWER_OFFSETS[k][0], oz = FLOWER_OFFSETS[k][1];
        const fl = mesh(bloomGeo, M_PARK_BLOOM, ox, 0, oz);
        patch.add(fl);
      }
    });

    // Shore planting uses the same seasonal materials as the reserve canopy.
    const shoreDetails = new THREE.Group(); reserveGroup.add(shoreDetails);
    [[-2.35, -0.65], [1.95, 0.68], [0.95, -1.55], [-1.55, 1.37]].forEach(function (p, patchIndex) {
      const patch = new THREE.Group(); patch.position.set(p[0], Y_GRASS, p[1]); shoreDetails.add(patch);
      registerReservePlanting(patch, patchIndex / 4, "low", patchIndex < 2);
      for (let j = 0; j < 5; j++) {
        const x = Math.sin(j * 2.4) * 0.15;
        const z = Math.cos(j * 2.4) * 0.12;
        const h = 0.24 + (j % 3) * 0.09;
        patch.add(mesh(new THREE.CylinderGeometry(0.012, 0.019, h, 5), M_PARK_LEAF, x, h / 2, z, false, false));
        patch.add(mesh(new THREE.CylinderGeometry(0.025, 0.025, 0.10, 6), M_WOOD, x, h, z, false, false));
      }
      packRigidModel(patch);
    });
    [[-1.5, -0.65], [1.15, 0.40], [0.85, -0.85]].forEach(function (p) {
      const pad = mesh(new THREE.CylinderGeometry(0.14, 0.14, 0.014, 10), M_PARK_LEAF, p[0], Y_GRASS + 0.058, p[1], false, false);
      pad.scale.z = 0.8; shoreDetails.add(pad);
      shoreDetails.add(mesh(bloomGeo, M_PARK_BLOOM, p[0] + 0.035, Y_GRASS + 0.095, p[1], false, false));
    });
    // Keep shore clusters separate so secondary planting can gently recede.
    ...
    function addPine(px, pz, h, group, rank) {
      ...
      const isReserve = belongsToReserve(px, pz, group);
      const pineMat = isReserve ? M_PARK_PINE : M_PINE;
      g.crowns = [];
      [[0.56, 0.78, 0.75], [0.43, 0.76, 1.15], [0.28, 0.71, 1.53]].forEach(function (p, i) {
        const crown = mesh(new THREE.ConeGeometry(p[0] * h, p[1] * h, 7), pineMat, i === 1 ? 0.04 : 0, p[2] * h, 0);
        crown.rotation.y = i * 0.37 + px * 0.1; g.add(crown); g.crowns.push(crown);
      });
      if (!isReserve) packRigidModel(g);
      (group || reserveGroup).add(g);
      if (isReserve) {
        g.crowns.forEach(function (c) { c.userData.parkBaseScale = c.scale.clone(); });
        registerReservePlanting(g, rank === undefined ? 0 : rank, "tree", rank === undefined || rank <= 0.18);
      }
      return g;
    }
    function addDecid(px, pz, s, group, rank) {
      ...
      const isReserve = belongsToReserve(px, pz, group);
      const leafMat = isReserve ? M_PARK_LEAF : M_LEAF;
      const leafLightMat = isReserve ? M_PARK_LEAF_LIGHT : M_LEAF_LIGHT;
      g.crowns = [];
      [[0, 1.40, 0, 0.41], [-0.31, 1.13, 0.04, 0.37], [0.32, 1.27, -0.04, 0.36],
       [0.07, 1.13, 0.29, 0.35], [-0.06, 1.26, -0.25, 0.33]].forEach(function (p, i) {
        const crown = mesh(new THREE.DodecahedronGeometry(p[3] * s, 0), i % 3 ? leafMat : leafLightMat, p[0] * s, p[1] * s, p[2] * s);
        crown.scale.y = 1.12; crown.rotation.y = px * 0.17 + i; g.add(crown); g.crowns.push(crown);
      });
      if (!isReserve) packRigidModel(g);
      (group || reserveGroup).add(g);
      if (isReserve) {
    ...
    // ────────────────────────────────────────────────────────────────
    // 🌱 10d. NATURE RESERVE: MONTHLY SPENDING PACE DYNAMICS
    // ────────────────────────────────────────────────────────────────
    ...
    function applyParkHealth(h) {
      ...
      // Four coordinated ecological states; the financial mapping stays in ReserveStateService.
      let targetMeadow, targetPine, targetLeaf, targetLeafLight, targetBloom, crownScale;
      let density, flowerDensity, lowScale;

      if (parkHealthValue >= 0.90) {
        // Calm (קצב רגוע)
        targetMeadow    = C(0x39894D);
        targetPine      = C(0x1B5E34);
        targetLeaf      = C(0x349946);
        targetLeafLight = C(0x58B648);
        targetBloom     = C(0xF8FAFC);
        crownScale      = 1.16;
        density = 1; flowerDensity = 1; lowScale = 1.12;
      } else if (parkHealthValue >= 0.65) {
        // Balanced (קצב מאוזן)
        targetMeadow    = C(0x66B63E);
        targetPine      = C(0x2D8048);
        targetLeaf      = C(0x4EAA50);
        targetLeafLight = C(0x78BF58);
        targetBloom     = C(0xFACC15);
        crownScale      = 1.00;
        density = 0.95; flowerDensity = 0.90; lowScale = 1;
      } else if (parkHealthValue >= 0.38) {
        // Active (קצב מעט גבוה)
        targetMeadow    = C(0x91B653);
        targetPine      = C(0x3E8E42);
        targetLeaf      = C(0x6EAF3C);
        targetLeafLight = C(0x94CA4E);
        targetBloom     = C(0xF59E0B);
        crownScale      = 0.92;
        density = 0.82; flowerDensity = 0.65; lowScale = 0.92;
      } else {
        // Busy (קצב מהיר)
        targetMeadow    = C(0xB3AF67);
        targetPine      = C(0x5A7A3E);
        targetLeaf      = C(0xB28628);
        targetLeafLight = C(0xC69A36);
        targetBloom     = C(0xD97706);
        crownScale      = 0.82;
    ...
    function setParkLook(meadow, pine, leaf, leafLight, bloom, crownScale, density, flowerDensity, lowScale) {
      ...
      for (let i = 0; i < parkPlantings.length; i++) {
        ...
        p.wanted = p.structural || p.rank < (p.kind === "flowers" ? flowerDensity : density);
        p.environmentScale = p.kind === "low" ? lowScale : 1;
        if (p.wanted) p.obj.visible = true;
        if (p.isReserve && p.obj.crowns) {
          for (let j = 0; j < p.obj.crowns.length; j++) {
            p.obj.crowns[j].scale.copy(p.obj.crowns[j].userData.parkBaseScale).multiplyScalar(crownScale);
          }
        }
      }
    }
    ...
    function stepPlantings(dt) {
      for (let i = 0; i < parkPlantings.length; i++) {
        ...
        const target = p.wanted ? p.environmentScale : 0;
        const cur = p.obj.scale.y / p.baseScale.y;
        if (p.wanted && Math.abs(cur - target) < 0.001) {
          if (cur !== target) p.obj.scale.copy(p.baseScale).multiplyScalar(target);
          continue;
        }
        const next = cur + (target - cur) * Math.min(1, dt * 4.5);
        if (!p.wanted && next < 0.02) { p.obj.visible = false; p.obj.scale.setScalar(0.01); continue; }
        p.obj.scale.copy(p.baseScale).multiplyScalar(Math.max(0.01, next));
      }
    }

    // Preserve authored ranks and structural ownership; street trees never enter this list.
    applyParkHealth(HEALTHY_PARK);
    parkPlantings.forEach(function (p) {
      p.obj.visible = p.wanted;
      p.obj.scale.copy(p.baseScale).multiplyScalar(p.wanted ? p.environmentScale : 0.01);
    });
    ...
    function addCitizen(color, pants, hatColor, walkPath, phrases, englishPhrases, withDog, customSpeed, appearanceKey) {
      // When appearanceKey is provided the diversity system activates; otherwise legacy behavior.
      let appearance = null;
      if (appearanceKey) {
        appearance = chooseAppearanceForSlot(appearanceKey, recentPopulationProfiles);
        recentPopulationProfiles.push(appearance);
        if (recentPopulationProfiles.length > RECENT_PROFILE_WINDOW) recentPopulationProfiles.shift();
      }
      const g = appearance
        ? makeFigure({ appearance: appearance })
        : makeFigure({ shirt: color, pants: pants, cap: hatColor,
            dark: walkingCitizens.length % 3 === 1,
            bag: withDog ? null : (walkingCitizens.length % 2 ? 0xA67F52 : null) });
      ...
      if (walkPath && walkPath.length > 1) {
        ...
        const baseSpeed = customSpeed || 1.15;
        const motionMult = appearance ? appearance.motion.speedMult : 1.0;
        walkingCitizens.push({
          ...
          pIdx: 0,
          t: 0,
          speed: baseSpeed * motionMult,
          motion: appearance ? appearance.motion : null,
    ...
    ], ["חוצה בזהירות במעבר חצייה 🚶", "העיר תוססת היום! ✨"], ["Crossing at the zebra walk 🚶", "Lively city today! ✨"], false, 0.38, 'city:crosswalk:ped');
    ...
    ], ["ריצת בוקר מסביב למרכז 🏃‍♂️", "כושר גופני וכושר פיננסי 💪"], ["Morning 5k jog 🏃‍♂️", "Fit body, fit finances 💪"], false, 0.65, 'city:jogger');

    // 5. Dog Walker near park
    addCitizen(null, null, null, [
      {x: -3.2, z: -7.4}, {x: 3.2, z: -7.4}, {x: 3.2, z: -11.0}, {x: -3.2, z: -11.0}
    ], ["טיול עם הכלב בפארק 🐕", "השכונה שקטה ונעימה 🌳"], ["Walking the dog in the park 🐕", "Peaceful neighbourhood 🌳"], true, 0.34, 'city:dogwalk');

    // ────────────────────────────────────────────────────────────────
    // Two park neighbours use the same actor pool and authored court-side paths.
    ['city:park:nbr:0', 'city:park:nbr:1'].forEach(function (aKey, i) {
      addCitizen(null, null, null, [
    ...
    stage.addEventListener("pointerdown", function (e) {
      if (activeExpenseFocus && !activeExpenseFocus.cameraCancelled) {
        activeExpenseFocus.cameraCancelled = true;
        targetCam.lookX = currentCam.lookX;
        targetCam.lookY = currentCam.lookY;
        targetCam.lookZ = currentCam.lookZ;
        targetCam.zoom = currentCam.zoom;
      }
    ...
    // Subtle focus pulse ring for newly added transactions
    const expensePulseRing = new THREE.Mesh(
      new THREE.RingGeometry(1.20, 1.48, 36),
      new THREE.MeshBasicMaterial({ color: C(0xCBD5E1), transparent: true, opacity: 0.0, depthWrite: false })
    );
    expensePulseRing.rotation.x = -Math.PI / 2;
    expensePulseRing.visible = false;
    root.add(expensePulseRing);
    ...
    // ────────────────────────────────────────────────────────────────
    // 💸 EXPENSE BUILDING FOCUS (Creation-Only Spatial Interaction)
    // Connecting a transaction directly to its physical impact on the city skyline.
    // ────────────────────────────────────────────────────────────────
    let activeExpenseFocus = null;
    let lastHandledExpenseToken = null;
    let pendingExpenseFocusRequest = null;

    function cancelExpenseFocus() {
      if (!activeExpenseFocus) return;
      const f = activeExpenseFocus;
      if (f.bubbleEl && f.bubbleEl.parentNode) {
        f.bubbleEl.parentNode.removeChild(f.bubbleEl);
      }
      expensePulseRing.visible = false;
      if (f.shellObj) f.shellObj.scale.set(1, 1, 1);
      activeExpenseFocus = null;
    }

    function focusDioramaBuilding(buildingId, amount, token, formattedAmount, isRefund) {
      if (!buildingId || !token) return false;
      if (token === lastHandledExpenseToken) return true;
      lastHandledExpenseToken = token;

      // If diorama data is not yet applied or interactiveBuildings is empty, queue it
      if (!interactiveBuildings || interactiveBuildings.length === 0) {
        pendingExpenseFocusRequest = {
      ...
      const body = bodyOf(buildingId);
      const rootObj = buildingRoots[buildingId] || body;
      if (!body && !rootObj) {
        console.warn("[ExpenseFocus] Building not found in scene:", buildingId);
        return false;
      }

      // If an existing focus is running, cleanly cancel it (Latest explicit user action wins)
      cancelExpenseFocus();

      const targetObj = body || rootObj;
      const w = new THREE.Vector3();
      targetObj.getWorldPosition(w);

      // Find top of building for spatial bubble anchoring
      let topY = w.y + 1.2;
      if (rootObj) {
        const box = new THREE.Box3().setFromObject(rootObj);
        if (isFinite(box.max.y) && box.max.y > w.y) {
          topY = box.max.y;
        }
      }
      const bubbleWorldAnchor = new THREE.Vector3(w.x, topY + 0.35, w.z);

      const isReducedMotion = !!(window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)").matches);

      // Camera target: smooth glide to building from current camera angle with subtle ~12% zoom
      const az = currentCam.az;
      const fx = Math.cos(az);
      const fz = Math.sin(az);
      const groundOffset = 1.6;
      const targetLookX = clamp(w.x + groundOffset * fx, -PAN_LIMIT, PAN_LIMIT);
      const targetLookY = Math.max(0.2, (w.y || 0) + 0.35);
      const targetLookZ = clamp(w.z + groundOffset * fz, -PAN_LIMIT, PAN_LIMIT);
      const targetZoom = clamp((currentCam.zoom || 1.0) * 1.12, ZOOM_MIN, ZOOM_MAX);

      if (isReducedMotion) {
        targetCam.lookX = targetLookX;
        targetCam.lookY = targetLookY;
        targetCam.lookZ = targetLookZ;
        targetCam.zoom = targetZoom;
        currentCam.lookX = targetLookX;
        currentCam.lookY = targetLookY;
        currentCam.lookZ = targetLookZ;
        currentCam.zoom = targetZoom;
      } else {
        targetCam.lookX = targetLookX;
        targetCam.lookY = targetLookY;
        targetCam.lookZ = targetLookZ;
      ...
      // Create amount bubble DOM element in overlays container
      const bubbleEl = document.createElement("div");
      bubbleEl.className = "diorama-expense-bubble";
      const isNeg = !!isRefund || amount < 0;
      const sign = isNeg ? "−" : "+";
      const signClass = isNeg ? "sign-neg" : "sign-pos";
      let amountText = formattedAmount;
      if (!amountText || amountText === "null" || amountText === "undefined") {
        const absAmt = Math.abs(amount);
        const formattedNum = (absAmt % 1 === 0) ? absAmt.toFixed(0) : absAmt.toFixed(2);
        amountText = "₪" + formattedNum;
      }
      if (amountText.startsWith("+") || amountText.startsWith("-") || amountText.startsWith("−")) {
        amountText = amountText.substring(1).trim();
      }
      const signSpan = document.createElement("span");
      signSpan.className = signClass;
      signSpan.textContent = sign;
      const amountSpan = document.createElement("span");
      amountSpan.textContent = amountText;
      bubbleEl.textContent = "";
      bubbleEl.appendChild(signSpan);
      bubbleEl.appendChild(amountSpan);
      (overlaysContainer || stage || document.body).appendChild(bubbleEl);

      activeExpenseFocus = {
        token: token,
        buildingId: buildingId,
        targetObj: targetObj,
        shellObj: (targetObj.userData && targetObj.userData.shell)
          ? targetObj.userData.shell
          : ((rootObj && rootObj.userData && rootObj.userData.shell) ? rootObj.userData.shell : rootObj),
        worldAnchor: bubbleWorldAnchor,
        bubbleEl: bubbleEl,
        t: 0,
        pulseDelay: isReducedMotion ? 0.0 : 0.35,
        pulseDuration: 0.75,
        pulseStarted: false,
        bubbleDelay: isReducedMotion ? 0.05 : 0.38,
        bubbleShown: false,
        bubbleClosing: false,
        totalDuration: isReducedMotion ? 2.8 : 3.8,
    ...
    window.focusDioramaBuilding = focusDioramaBuilding;
    ...
    window.updateDioramaData = function (data) {
      ...
      try {
        ...
        if (pendingExpenseFocusRequest) {
          const req = pendingExpenseFocusRequest;
          pendingExpenseFocusRequest = null;
          focusDioramaBuilding(req.buildingId, req.amount, req.token, req.formattedAmount, req.isRefund);
        }
      } catch (err) {
    ...
    function loop(now) {
      ...
      // Step expense spatial building focus
      if (activeExpenseFocus) {
        const f = activeExpenseFocus;
        f.t += dt;

        // 1. User gesture cancellation: stop driving camera
        if (!f.cameraCancelled && isDragging) {
          f.cameraCancelled = true;
          targetCam.lookX = currentCam.lookX;
          targetCam.lookY = currentCam.lookY;
          targetCam.lookZ = currentCam.lookZ;
          targetCam.zoom = currentCam.zoom;
        ...
        // 2. Pulse step: subtle ground ring + gentle shell scale
        if (!f.pulseStarted && f.t >= f.pulseDelay) {
          f.pulseStarted = true;
          expensePulseRing.position.set(f.centerPos.x, Y_WALK + 0.06, f.centerPos.z);
          expensePulseRing.visible = true;
          expensePulseRing.material.opacity = 0.0;
          expensePulseRing.scale.setScalar(1.0);
        }
        if (f.pulseStarted) {
          const pt = f.t - f.pulseDelay;
          const pk = Math.min(1, pt / f.pulseDuration);
          if (pk < 1) {
            const op = Math.sin(pk * Math.PI) * 0.45;
            expensePulseRing.material.opacity = op;
            expensePulseRing.scale.setScalar(1.0 + pk * 0.30);
            if (!f.isReducedMotion && f.shellObj) {
              const squash = Math.sin(pk * Math.PI) * 0.04;
              f.shellObj.scale.y = 1.0 + squash;
              f.shellObj.scale.x = 1.0 - squash * 0.25;
              f.shellObj.scale.z = 1.0 - squash * 0.25;
            }
          } else {
            expensePulseRing.visible = false;
            if (f.shellObj) f.shellObj.scale.set(1, 1, 1);
        ...
        // 3. Amount bubble step: fade in, hold, drift and fade out
        if (!f.bubbleShown && f.t >= f.bubbleDelay) {
          f.bubbleShown = true;
          f.bubbleEl.classList.add("active");
        }
        if (f.bubbleShown) {
          if (!f.bubbleClosing && f.t >= (f.totalDuration - 0.45)) {
            f.bubbleClosing = true;
            f.bubbleEl.classList.remove("active");
            f.bubbleEl.classList.add("closing");
          }

          // Project 3D anchor above building to 2D screen coordinates
          const proj = new THREE.Vector3().copy(f.worldAnchor).project(camera);
          const inFront = proj.z >= -1 && proj.z <= 1;
          if (inFront) {
            f.bubbleEl.style.display = "inline-flex";
            const sx = (proj.x * 0.5 + 0.5) * viewportWidth;
            const sy = (-proj.y * 0.5 + 0.5) * viewportHeight;
            f.bubbleEl.style.left = sx + "px";
            f.bubbleEl.style.top = sy + "px";
          } else {
            f.bubbleEl.style.display = "none";
          }
        }

        // 4. Lifecycle cleanup
        if (f.t >= f.totalDuration) {
          cancelExpenseFocus();
        }
      }
      ...
      stepCityEncounters(dt);

      // Step walking citizens
      walkingCitizens.forEach(function (c) {
        if (!c.obj.visible || c.encounter) return;
        c.t += dt * (c.speed || 1.0);
        ...
        const walkCycle = now * 0.0055 * Math.max(0.6, (c.speed || 0.35) / 0.35) * cadence + (c.phase || 0);
        const legSwing  = Math.sin(walkCycle) * 0.38 * swingAmt;
