...
<body>
  ...
  <script>
    ...
    [[-1.8, 1.1], [-1.2, 1.8], [0.8, 1.8], [1.9, 1.2], [-0.9, -1.8], [0.5, -2.1], [1.8, -1.2], [-2.3, -0.8]].forEach(function (bp) {
      for (let k = 0; k < 4; k++) {
        const ox = FLOWER_OFFSETS[k][0], oz = FLOWER_OFFSETS[k][1];
        const fl = mesh(bloomGeo, M_PARK_BLOOM, bp[0] + ox, Y_GRASS + 0.04, bp[1] + oz);
        parkBloomGroup.add(fl);
      }
    });
    ...
    [[-2.35, -0.65], [1.95, 0.68], [0.95, -1.55], [-1.55, 1.37]].forEach(function (p) {
      for (let j = 0; j < 5; j++) {
        const x = p[0] + Math.sin(j * 2.4) * 0.15;
        const z = p[1] + Math.cos(j * 2.4) * 0.12;
        const h = 0.24 + (j % 3) * 0.09;
        shoreDetails.add(mesh(new THREE.CylinderGeometry(0.012, 0.019, h, 5), M_PARK_LEAF, x, Y_GRASS + h / 2, z, false, false));
        shoreDetails.add(mesh(new THREE.CylinderGeometry(0.025, 0.025, 0.10, 6), M_WOOD, x, Y_GRASS + h, z, false, false));
      }
    });
    ...
    function addPine(px, pz, h, group, rank) {
      ...
      const isReserve = (!group || group === reserveGroup);
      ...
      if (rank !== undefined) parkPlantings.push({ obj: g, rank: rank, isReserve: isReserve });
      return g;
    }
    ...
    function applyParkHealth(h) {
      ...
      // 4 Distinct Visual States based on monthly spending pace:
      // Calm:     h >= 0.90 (rich deep forest emerald meadow, pure white & soft lilac blossoms, 1.12x full canopy)
      // Balanced: 0.65 <= h < 0.90 (vibrant natural meadow green, sunny yellow & white wildflowers, 1.0x canopy)
      // Active:   0.38 <= h < 0.65 (warm summer sunlit yellow-green, marigold/orange flower patches, 0.94x canopy)
      // Busy:     h < 0.38 (warm golden-ochre autumn prairie, rustic amber oak tones, 0.88x canopy)
      let targetMeadow, targetPine, targetLeaf, targetLeafLight, targetBloom, crownScale;

      if (parkHealthValue >= 0.90) {
        // Calm (קצב רגוע)
        targetMeadow    = C(0x4A9E48);
      ...
      for (let i = 0; i < parkPlantings.length; i++) {
        ...
        if (p.isReserve && p.obj.crowns) {
          for (let j = 0; j < p.obj.crowns.length; j++) {
            p.obj.crowns[j].scale.setScalar(crownScale);
          }
        }
      }
    }
    ...
    parkPlantings.forEach(function (p) { p.wanted = true; p.obj.scale.setScalar(1); p.obj.visible = true; });
